          ...
          for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
              if (qrcode.isDark(row, col)) {
                ...
                var h = Math.ceil((row + 1) * tileH) - Math.floor(row * tileH); //   let node = cc.instantiate(cc.find("testNode", this.node));
                ...
                let posx = Math.round(col * tileW) - this.node.getComponent(cc.UITransform).width / 2;
                let posy = Math.round(col * tileH) - this.node.getComponent(cc.UITransform).height / 2;
                ctx.fillRect(posx, posy, w * 5, h * 5); //   ctx.fillRect();
...
//# sourceMappingURL=b4b15801e761bd27dd83e007d932a81917da005d.js.map