          ...
          for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
              if (qrcode.isDark(row, col)) {
                ...
                let posx = Math.round(col * tileW) - this.node.getComponent(cc.UITransform).width + w / 2;
                let posy = Math.round(col * tileH) - this.node.getComponent(cc.UITransform).height + h / 2;
                ctx.rect(posx, posy, w, h); //   ctx.fillRect();
...
//# sourceMappingURL=864d428562a662346499e2d08b098d70a12a7f70.js.map