          ...
          for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
              if (qrcode.isDark(row, col)) {
                ...
                let posx = Math.round(col * tileW) - this.node.getComponent(cc.UITransform).width / 2 + w / 2;
                let posy = Math.round(row * tileH) - this.node.getComponent(cc.UITransform).height / 2 + h / 2;
...
//# sourceMappingURL=071acc9700d1926b3a9ad2f45443464246f6513d.js.map