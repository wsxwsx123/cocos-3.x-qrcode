    ...
    execute: function () {
      ...
      _export("test", test = (_dec = ccclass("test"), _dec(_class = class test extends Component {
        ...
        QRCreate(ctx, url) {
          ...
          var tileH = this.node.getComponent(cc.UITransform).height / roll; // draw in the Graphics
          ...
          for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
              if (qrcode.isDark(row, col)) {
                ...
                var posy = Math.round(row * tileH) - this.node.getComponent(cc.UITransform).height / 2;
                ctx.rect(posx, posy, tileW, tileH + 0.5);
                ctx.fillColor = cc.Color.WHITE;
                ctx.fill();
              }
            }
...
//# sourceMappingURL=6ad97bd5b538fdf1a32d729c71a4ee6a8c4e6451.js.map