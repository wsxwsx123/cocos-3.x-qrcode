          ...
          for (var row = 0; row < qrcode.getModuleCount(); row++) {
            for (var col = 0; col < qrcode.getModuleCount(); col++) {
              if (qrcode.isDark(row, col)) {
                ...
                var node = cc.instantiate(cc.find("testNode", this.node));
                this.node.addChild(node);
                node.setPosition(cc.v3(Math.round(col * tileW) - this.node.getComponent(cc.UITransform).width / 2 + w / 2, Math.round(row * tileH) - this.node.getComponent(cc.UITransform).height / 2 + h / 2));
...
//# sourceMappingURL=a66adfe90996fdbb96c3559245789ee71917a44f.js.map